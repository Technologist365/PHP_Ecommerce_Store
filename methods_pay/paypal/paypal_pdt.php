...
if(file_exists('../../includes/db_config.php') == TRUE)
{
	...
	if($installed == '1')
	{
		...
		//Grab the PDT token from the database, also whether we are live or not
		$paypal_sql = mysql_query("SELECT method_id, account, currency, token, live FROM methods WHERE name='Paypal'");
		$paypal = mysql_fetch_array ($paypal_sql);
		
		$paypal_token = $paypal['token'];//Token needed for PDT response to Paypal
		$paypal_live = $paypal['live'];//Is this method live or not?
		$method_id = $paypal['method_id']; //ID number for this particular method
		$paypal_business = $paypal['account']; //Account that the money should be getting sent to
		$paypal_currency = $paypal['currency']; //Currency that the store uses
			...
			if (!$fp) 
				{
				echo "<h1>Posting back to PayPal failed!</h1><br> If your purchases do not show on your account ".
					 "after 30 minutes or an hour, please contact the site administrator to confirm the payment ".
					 "has successfully went through.";
				} 
			else 
				{
				//echo "Post Back OK!<br>";
				fputs ($fp, $header . $req);
				// read the body data
				$res = '';
				$headerdone = false;
				...
				if (strcmp ($lines[0], "SUCCESS") == 0) 
					...
					//We might want to record this, not sure, so I wont, for now anyways.
					...
					$tran_id = $keyarray["txn_id"];
					
					$transaction_sql = mysql_query("SELECT tran_num FROM transactions WHERE tran_id='$tran_id' AND method_id='$method_id'");
					...
					if($previous_tran != NULL)
						...
						if not, contact the administrator for further assistance';
						}
					else
						{
						//Store the transaction id in the database
						mysql_query("INSERT INTO transactions (tran_id, method_id) VALUES('$tran_id', '$method_id')") or die (mysql_error());
						}
						
					//Make sure the payment has already completed
					...
					/*Make sure the currency is what it is supposed to be.
						5 Euros and 5 Dollars are different amounts.*/
					...
					if("$business" != "$paypal_business")
						{
						//They don't match, return an error
						echo "Business mismatch<br>";
						echo "$business<br>";
						echo "$paypal_business<br>";
						$error = 'An uknown error has occured while processing your transaction.';
						}
					
					//Make sure that the gross payment amount is equal to what everything in the cart costs.
					$payment_amount = $keyarray["payment_gross"];
