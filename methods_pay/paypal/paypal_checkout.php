...
if(file_exists('../../includes/db_config.php') == TRUE)
{
	...
	if($installed == '1')
	{
		...
		$shipping_sql = mysql_query("SELECT name, phone, email, address1, address2, country, city, state, zip, special FROM carts WHERE cart_id=$cart_id");
		...
		//Page Content
		$smarty->assign('page_name', 'Paypal Checkout');
		$smarty->assign('long_descrip', "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Please review your order and shipping information and after confirming that all items and information are correct use the button below to continue to Paypal to pay for your items. <br><br>
		...
		//Grab the variables from the database about this payment method
		$paypal_sql = mysql_query("SELECT account, currency, checkout_icon, live FROM methods WHERE name='Paypal'");
		$paypal = mysql_fetch_array ($paypal_sql);
		
		$smarty->assign('paypal_business', $paypal['account']);//Business - Email address on your paypal account
		$smarty->assign('paypal_currency', $paypal['currency']);//Currency Code - Possible values are "USD", "EUR", "GBP", "CAD", "JPY"
		$smarty->assign('checkout_icon', $paypal['checkout_icon']); //Icon for the checkout, stored in the database so it can be changed
		
		/*Live Store - This is a 1 or 0 value, 1 for yes, 0 for no 
		This sets whether your store should use the live paypal servers or the sandbox servers.
		Please note that you must sign up for the sandbox and create at least 1 sandbox business 
			account and 1 sandbox user account to be able to test using sandbox mode.*/
		$paypal_live = $paypal['live'];

		if($paypal_live == '1')
			{
			$smarty->assign('paypal_url', 'https://www.paypal.com');
			}
		else
			{
			$smarty->assign('paypal_url', 'https://www.sandbox.paypal.com');
			}
		...
		//$smarty->display('category_top.tpl');
				...
				elseif (!mysql_num_rows($cart_query))
					...
					$smarty->display('checkout_begin.tpl');
				
					//Output the beginning of the form
					$smarty->display('methods_pay/paypal/paypal_begin.tpl');
					...
					while($cart_rows = mysql_fetch_array($cart_query))
						...
						$smarty->assign('prod_license',$price_info['name']);//License type
						...
						$smarty->display('checkout_data.tpl');
					...
					//Button to submit to Paypal
					//Also cost and shipping values to submit to paypal
					$smarty->display('methods_pay/paypal/paypal_end.tpl');
					
					//The end of the table and form
					$smarty->display('checkout_end.tpl');
					}
